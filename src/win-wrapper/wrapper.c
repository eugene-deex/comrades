...
BOOL (*pAllowSetForegroundWindow)( DWORD dwProcessId );
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	int len = GetModuleFileNameW(NULL, exePath, maxLenSafe);

	pAllowSetForegroundWindow = (void *)GetProcAddress( GetModuleHandle( "user32.dll" ), "AllowSetForegroundWindow" );
	pGetProcessId = (void *)GetProcAddress( GetModuleHandle( "kernel32.dll" ), "GetProcessId" );
	...
	// .. and start the wrapped executable
	{
		...
		info.pid = pGetProcessId(pi.hProcess);
		info.hwnd = 0;
		pAllowSetForegroundWindow(info.pid);
