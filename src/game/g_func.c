...
void Move_Done( edict_t *ent )
{
	...
	if( ent->parent )
		VectorCopy( ent->parent_ent->velocity, ent->velocity );
...
void Move_Final( edict_t *ent )
{
	...
	if( ent->parent )
		VectorAdd( ent->velocity, ent->parent_ent->velocity, ent->velocity );
...
Think_SpawnDoorTrigger(edict_t *ent)
{
	...
	if(ent->parent)
	{
		...
		if( ent->parent_ent ) parent_init( ent->parent_ent );
	}
