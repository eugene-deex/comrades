...
void MoveRiders_r( edict_t *platform, edict_t *ignore, vec3_t move, vec3_t amove, qboolean turn )
{
	int	i;
	edict_t	*rider;
	...
	for( e = 1; e < globals.num_edicts; e++, check++ )
	{
		...
		if ((pusher->movetype == MOVETYPE_PUSH) || (check->groundentity == pusher))
		{
			...
			if( !block )
			{
				...
				MoveRiders_r( check, NULL, move3, amove, false );
				continue;
			}
...
void SV_RestorePosition( edict_t *ent )
{
	// restore from backup
	VectorCopy( ent->move_origin, ent->s.origin );
	VectorCopy( ent->move_angles, ent->s.angles );
	gi.linkentity(ent);
}

void SV_CallBlocked( edict_t *self, edict_t *obstacle )
{
	if( !self ) return;

	// tell my parent about blocking
	if( self->parent_ent && self->parent_ent != self )
	{
		// return the parent on initial position
		SV_RestorePosition( self->parent_ent );
		SV_CallBlocked( self->parent_ent, obstacle );
	}

//	gi.dprintf( "%s call blocking\n", self->classname );
...
G_RunEntity(edict_t *ent)
{
	...
	// run parent first
	if( ent->parent_ent )
		G_RunEntity( ent->parent_ent );

	if( ent->runframe == level.framenum )
		return; // already handled this frame

	ent->runframe = level.framenum;

	// make backup in case some child will be blocked
	VectorCopy( ent->s.origin, ent->move_origin );
	VectorCopy( ent->s.angles, ent->move_angles );

	// first we should compute movement for childs for this frame
	if( ent->child_next && ( ent->child_next->parent_ent == ent ))
