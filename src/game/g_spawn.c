...
void ED_CallSpawn( edict_t *ent )
{
	...
	// preserve original angles for parent stuff 
...
void SpawnEntities( const char *mapname, char *entities, const char *spawnpoint )
{
	...
	for( i = 1, ent = g_edicts + i; i < globals.num_edicts; i++, ent++ )
	{
		...
		if( ent->parent_ent )
			continue;
		ent->parent_ent = G_Find( NULL, FOFS( targetname ), ent->parent );
		// Make sure that we can really attach this guy. This check
		// allows us to have parent with same targetname as
		// other entities
		while( ent->parent_ent &&
			( (strcmp( ent->parent_ent->classname, "func_train" ))     &&
			  (strcmp( ent->parent_ent->classname, "func_door" ))) )
			 ent->parent_ent = G_Find( ent->parent_ent, FOFS( targetname ), ent->parent );
		if( ent->parent_ent )
			parent_init( ent->parent_ent );
	}
}
