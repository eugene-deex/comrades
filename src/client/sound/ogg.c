...
OGG_InitTrackList(void)
{
	...
	int i, potMusicDirIdx;

	for (i=0; i<MAX_NUM_OGGTRACKS; ++i)
	{
	...
	else
	{
		...
		sprintf(gameMusicDir, "%s/music/", gameCvar->string);
	...
	for (potMusicDirIdx = 0; potMusicDirIdx < sizeof(potMusicDirs)/sizeof(potMusicDirs[0]); ++potMusicDirIdx)
	{
		...
		for (rawPath = FS_GetNextRawPath(NULL); rawPath != NULL; rawPath = FS_GetNextRawPath(rawPath))
		{
			char fullMusicPath[MAX_OSPATH] = {0};
			char testFileName[MAX_OSPATH];
			int gogTrack;

			sprintf(fullMusicPath, "%s/%s", rawPath, musicDir);
			...
			sprintf(testFileName, "%s02.ogg", fullMusicPath);
			...
			if(Sys_IsFile(testFileName))
			{
				...
				{
					sprintf(testFileName, "%s%02i.ogg", fullMusicPath, i);
			...
			gogTrack = getMappedGOGtrack(8, gameType);

			sprintf(testFileName, "%sTrack%02i.ogg", fullMusicPath, gogTrack);
...
OGG_PlayTrack(int trackNo)
{
	...
	f = Q_fopen(ogg_tracks[trackNo], "rb");
...
OGG_Cmd(void)
{
	...
	else if (Q_stricmp(Cmd_Argv(1), "play") == 0)
	{
		...
		track = (int)strtol(Cmd_Argv(2), NULL, 10);
