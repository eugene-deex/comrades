...
R_DrawAliasFrameLerp(dmdl_t *paliashdr, float backlerp)
{
	...
	R_LerpVerts(paliashdr->num_xyz, v, ov, verts, lerp, move, frontv, backv);
		...
		while (1)
		{
			...
			total = count;
#if defined(_WIN32)
	vtx = _alloca( sizeof( GLfloat ) * 3 * total );
	tex = _alloca( sizeof( GLfloat ) * 2 * total );
	clr = _alloca( sizeof( GLfloat ) * 4 * total );
#else
	vtx = alloca( sizeof( GLfloat ) * 3 * total );
	tex = alloca( sizeof( GLfloat ) * 2 * total );
	clr = alloca( sizeof( GLfloat ) * 4 * total );
#endif
