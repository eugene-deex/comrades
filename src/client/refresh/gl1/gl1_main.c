...
#if defined(_WIN32)
	GLfloat *vtx = _alloca( sizeof( GLfloat ) * 3 * num_particles * 3 );
	GLfloat *tex = _alloca( sizeof( GLfloat ) * 2 * num_particles * 3 );
	GLfloat *clr = _alloca( sizeof( GLfloat ) * 4 * num_particles * 3 );
#else
	GLfloat *vtx = alloca( sizeof( GLfloat ) * 3 * num_particles * 3 );
	GLfloat *tex = alloca( sizeof( GLfloat ) * 2 * num_particles * 3 );
	GLfloat *clr = alloca( sizeof( GLfloat ) * 4 * num_particles * 3 );
#endif
...
#if defined(_WIN32)
	GLfloat *vtx = _alloca( sizeof( GLfloat ) * 3 * r_newrefdef.num_particles );
	GLfloat *clr = _alloca( sizeof( GLfloat ) * 4 * r_newrefdef.num_particles );
#else
	GLfloat *vtx = alloca( sizeof( GLfloat ) * 3 * r_newrefdef.num_particles );
	GLfloat *clr = alloca( sizeof( GLfloat ) * 4 * r_newrefdef.num_particles );
#endif
...
{
	int x, w, y, h;
	qboolean drawing_left_eye;
	qboolean stereo_split_tb;
	qboolean stereo_split_lr;
	...
	drawing_left_eye = gl_state.camera_separation < 0;
	stereo_split_tb = ((gl_state.stereo_mode == STEREO_SPLIT_VERTICAL) && gl_state.camera_separation);
	stereo_split_lr = ((gl_state.stereo_mode == STEREO_SPLIT_HORIZONTAL) && gl_state.camera_separation);
