...
R_FindFreeImage (void)
{
	...
	if (i == numr_images)
	{
		if (numr_images == MAX_RIMAGES)
			ri.Sys_Error(ERR_DROP, "R_FindFreeImage: Max images" );
		numr_images++;
	}
	image = &r_images[i];
...
R_LoadPic (char *name, byte *pic, int width, int height, imagetype_t type)
{
	...
	if (strlen(name) >= sizeof(image->name))
		ri.Sys_Error(ERR_DROP, "R_LoadPic: '%s' is too long", name);
	...
	if (!image->pixels[0])
	{
		ri.Sys_Error(ERR_FATAL, "R_LoadPic: Can't allocate image." );
...
R_InitImages (void)
{
	...
	if ( !table16to8 )
	{
		ri.Sys_Error(ERR_FATAL, "R_InitImages: Couldn't load pics/16to8.dat" );
		// code never returns after ERR_FATAL
		return;
	}
