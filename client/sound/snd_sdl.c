...
static void lpf_initialize(LpfContext* lpf_context, float gain_hf, int target_frequency)
{
	...
	cw = cos((k_2_pi * lpf_reference_frequency) / target_frequency);

	a = 0.0F;

	if (g < 0.9999F)
	{
		a = (1.0F - (g * cw) - sqrt((2.0F * g * (1.0F - cw)) -
...
SDL_BackendInit(void)
{
	...
	sndbits = (Cvar_Get("sndbits", "16", CVAR_ARCHIVE))->value;
	sndfreq = (Cvar_Get("s_khz", "44", CVAR_ARCHIVE))->value;
	sndchans = (Cvar_Get("sndchannels", "2", CVAR_ARCHIVE))->value;
	...
	sprintf(reqdriver, "%s=%s", "SDL_AUDIODRIVER", s_sdldriver->string);
	putenv(reqdriver);
