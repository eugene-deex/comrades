...
R_ReallocateMapBuffers (void)
{
	if (!r_cnumsurfs || r_outofsurfaces)
	{
		...
		if (r_cnumsurfs > SURFINDEX_MAX)
		{
			...
			R_Printf(PRINT_ALL, "R_ReallocateMapBuffers: Code has limitation to surfaces count.\n" );
		}

		lsurfs = malloc (r_cnumsurfs * sizeof(surf_t));
		if (!lsurfs)
		{
			R_Printf(PRINT_ALL, "R_ReallocateMapBuffers: Couldn't malloc %d bytes\n", (int)(r_cnumsurfs * sizeof(surf_t)));
			return;
		}
	...
	if (!r_numallocatedlights || r_outoflights)
	{
		...
		if (!blocklights)
		{
			R_Printf(PRINT_ALL, "R_ReallocateMapBuffers: Couldn't malloc %d bytes\n", (int)(r_numallocatedlights * sizeof(light_t)));
			return;
		}
	...
	if (!r_numallocatededges || r_outofedges)
	{
		...
		if (!r_edges)
		{
			R_Printf(PRINT_ALL, "R_ReallocateMapBuffers: Couldn't malloc %d bytes\n", (int)(r_numallocatededges * sizeof(edge_t)));
			return;
		}
	...
	if (!r_numallocatedverts || r_outofverts)
	{
		...
		if (!finalverts)
		{
			R_Printf(PRINT_ALL, "R_ReallocateMapBuffers: Couldn't malloc %d bytes\n", (int)(r_numallocatedverts * sizeof(finalvert_t)));
			return;
		}
	...
	if (!r_numallocatedtriangles || r_outoftriangles)
	{
		...
		if (!triangle_spans)
		{
			R_Printf(PRINT_ALL, "R_ReallocateMapBuffers: Couldn't malloc %d bytes\n", (int)(r_numallocatedtriangles * sizeof(spanpackage_t)));
			return;
		}
	...
	if (!r_numallocatededgebasespans || r_outedgebasespans)
	{
		...
		if (!edge_basespans)
		{
			R_Printf(PRINT_ALL, "R_ReallocateMapBuffers: Couldn't malloc %d bytes\n", (int)(r_numallocatededgebasespans * sizeof(espan_t)));
			return;
		}
...
RE_RenderFrame (refdef_t *fd)
{
	...
	{
		ri.Sys_Error(ERR_FATAL, "RE_RenderFrame: NULL worldmodel" );
	}
		...
		*/
		...
		else
		{
			if ( err == rserr_invalid_mode )
			{
				...
				R_Printf(PRINT_ALL, "RE_BeginFrame: could not set mode" );
			}
			else if ( err == rserr_invalid_fullscreen )
			{
				...
				R_Printf(PRINT_ALL, "RE_BeginFrame: fullscreen unavailable in this mode" );
				sw_state.prev_mode = r_mode->value;
			}
			else
			{
				ri.Sys_Error(ERR_FATAL, "RE_BeginFrame: Catastrophic mode change failure" );
			}
		}
	}
}
...
RE_InitContext(void *win)
{
	...
	sprintf(title, "Yamagi Quake II %s - Soft Render", YQ2VERSION);
...
SWimp_CreateRender(void)
{
	...
	if (!swap_buffers)
	{
		ri.Sys_Error(ERR_FATAL, "SWimp_CreateRender: Can't allocate swapbuffer." );
