...
CalcSurfaceExtents (msurface_t *s)
{
	...
	for (i=0 ; i<2 ; i++)
	{
		...
		{
			ri.Sys_Error(ERR_DROP, "CalcSurfaceExtents: Bad surface extents" );
		}
	}
}
...
Mod_LoadFaces (lump_t *l)
{
	...
	for ( surfnum=0 ; surfnum<count ; surfnum++, in++, out++)
	{
		...
		if (out->numedges < 3)
		{
			ri.Sys_Error(ERR_DROP, "Mod_LoadFaces: Surface with %d edges", out->numedges);
		}
		out->flags = 0;
...
Mod_LoadBrushModel(model_t *mod, void *buffer, int modfilelen)
{
	...
	surfEdgeCount = (float)header->lumps[LUMP_SURFEDGES].filelen / sizeof(int);
...
Mod_LoadAliasModel(model_t *mod, void *buffer, int modfilelen)
{
	...
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadAliasModel: %s has wrong version number (%i should be %i)", mod->name, version, ALIAS_VERSION);
	}

	ofs_end = LittleLong(pinmodel->ofs_end);
	if (ofs_end < 0 || ofs_end > modfilelen)
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadAliasModel: model %s file size(%d) too small, should be %d", mod->name, modfilelen, ofs_end);
	}
	...
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadAliasModel: model %s has a skin taller than %d", mod->name, MAX_LBM_HEIGHT);
	}

	if (pheader->num_xyz <= 0)
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadAliasModel: model %s has no vertices", mod->name);
	}

	if (pheader->num_xyz > MAX_VERTS)
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadAliasModel: model %s has too many vertices", mod->name);
	}

	if (pheader->num_st <= 0)
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadAliasModel: model %s has no st vertices", mod->name);
	}

	if (pheader->num_tris <= 0)
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadAliasModel: model %s has no triangles", mod->name);
	}

	if (pheader->num_frames <= 0)
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadAliasModel: model %s has no frames", mod->name);
	}
...
Mod_LoadSpriteModel(model_t *mod, void *buffer, int modfilelen)
{
	...
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadSpriteModel: %s has wrong version number (%i should be %i)", mod->name, sprout->version, SPRITE_VERSION);
	}

	if (sprout->numframes > MAX_MD2SKINS)
	{
		ri.Sys_Error(ERR_DROP, "Mod_LoadSpriteModel: %s has too many frames (%i > %i)", mod->name, sprout->numframes, MAX_MD2SKINS);
	}
